"""
============================================================
  Self-Healing Neural Networks v4.0
  Fixing Model Drift Without Retraining (PyTorch)
============================================================
  v4 FIXES (over v3):
  ① pos_weight=6.0 in healing BCE — fraud samples weighted 6×
  ② semi_sup_alpha: 0.70 → 0.85  (more real-label, less constraint)
  ③ lambda_lagrange: 0.80 → 0.40 (gentler symbolic pressure)
  ④ Heal only on Type A conflicts (MLP=Normal, Rule=Fraud)
     Type B is extremely rare and adds noise
  ⑤ Confidence-weighted conflict mask — only heal on samples
     where MLP is confidently wrong (prob < 0.25 for fraud cases)
  ⑥ Log bce_real vs constraint_loss separately per heal event
  ⑦ Final report includes Precision@Recall curve stats
============================================================
"""
...
CFG = {
    ...
    "fidi_window"      : 10,
    "fidi_threshold"   : 1.0,
    "conflict_prob_thr": 0.25,    # ↓ from 0.40 — confident MLP errors only
    "conflict_min"     : 3,
    "lambda_lagrange"  : 0.40,    # ↓ from 0.80 — gentler symbolic pressure
    "n_heal_steps"     : 5,
    "heal_lr"          : 0.003,
    "semi_sup_alpha"   : 0.85,    # ↑ from 0.70 — more real-label weight
    "pos_weight"       : 6.0,     # ★ NEW — fraud class upweighted 6×
}
...
class SymbolicRuleEngine:
    """
    V14 < -1.5 → Fraud
    FIX ④: Only use Type A conflicts for healing.
    Type B (MLP=Fraud, Rule=Normal) is too rare and noisy.
    """
    THRESHOLD = -1.5
    ...
    def type_a_mask(self, X, mlp_probs):
        """
        Type A: MLP confidently says Normal (prob < conflict_thr)
                but Rule says Fraud.
        FIX ⑤: confidence_thr=0.25 — only heal on strong disagreements.
        """
        thr       = CFG["conflict_prob_thr"]
        mlp_norm  = mlp_probs < thr          # MLP very confidently Normal
        rule_fraud = X[:, 7] < self.THRESHOLD
        return mlp_norm & rule_fraud
...
# ============================================================
# 7. CLASS-WEIGHTED SEMI-SUPERVISED HEALING LOSS  (core fix)
# ============================================================
...
def healing_loss(probs, real_labels, symbolic_labels,
    ...
    """
    FIX ①: pos_weight upweights fraud class in BCE.
    L = alpha * BCE_weighted(real) + (1-alpha) * lambda * BCE(symbolic)

    pos_weight=6.0 means:
        loss on a fraud sample = 6 × loss on a normal sample
    This prevents healing from collapsing recall.
    """
    # Weighted BCE — fraud samples count 6× more
    bce_weighted  = nn.BCEWithLogitsLoss(
        pos_weight=torch.tensor([pos_wt])
    )
    # Convert probs → logits for BCEWithLogitsLoss
    logits        = torch.log(probs / (1 - probs + 1e-8))
    real_loss     = bce_weighted(logits, real_labels)

    # Unweighted constraint term (symbolic direction only)
    constraint    = nn.BCELoss()(probs, symbolic_labels)

    return alpha * real_loss + (1.0 - alpha) * lam * constraint, \
           real_loss.item(), constraint.item()
...
def heal(model, X_batch, y_batch, symbolic_engine,
    ...
    """
    FIX ④: Only heal on Type A conflict samples.
    FIX ⑥: Log bce_real vs constraint_loss per step.
    """
    mask = symbolic_engine.type_a_mask(X_batch,
                                       model(X_batch).detach())

    # Fall back to full batch if mask is empty
    if mask.sum() < 2:
        X_heal = X_batch
        y_heal = y_batch
    else:
    ...
    for _ in range(steps):
        ...
        probs      = model(X_heal)
        loss, r, c = healing_loss(probs, y_heal, sym_labels)
        loss.backward()
    ...
    return traj_total, traj_real, traj_const
...
def stream_experiment(heal_model, base_model,
    ...
    for i in range(n_batch):
        ...
        with torch.no_grad():
            base_acc  = ((base_model(xb) > 0.5).float() == yb).float().mean().item()
        ...
        # Fraud fraction inside conflict samples
        ...
        print(f"  {i+1:>3}  {z:>5.2f}  {status:>10}  {n_conf:>4}  "
              f"{event:>10}  {base_acc:>6.3f}  {pre_acc:>7.3f}  "
              f"{post_acc:>7.3f}  {delta_str:>6}  "
              f"{bce_r_avg:>7.4f}  {bce_c_avg:>7.4f}")

        if event == "⚡ HEALED" and n_conf > 0:
            print(f"       └─ Conflicts={n_conf}  "
                  f"RealFraudFrac={frac:.1%}  "
...
def plot(log, clean_m, drift_m, healed_m):
    ...
    ax_style(ax3, "Healing Loss Decomposition (Real vs Constraint)")

    # ── 4. Pre vs Post heal ────────────────────────────────
    ax4 = fig.add_subplot(gs[1, 1])
    pre_vals  = [log["before_acc"][i] for i in healed_i]
    post_vals = [log["after_acc"][i]  for i in healed_i]
    if healed_i:
        x_h = np.arange(len(healed_i))
        ...
        ax4.set_xticks(x_h[::2])
        ax4.set_xticklabels([f"B{i+1}" for i in healed_i[::2]],
                            fontsize=7)
    ...
    # ── 5. Summary — Accuracy + F1 + Precision + Recall ───
    ...
    ax5.bar(x_m - 0.25, clean_v,  width=0.23, color=G,
            alpha=0.85, label="Clean")
    ax5.bar(x_m,        drift_v,  width=0.23, color=R,
            alpha=0.85, label="Drift")
    ax5.bar(x_m + 0.25, healed_v, width=0.23, color=B,
            alpha=0.85, label="Healed")
    ax5.set_xticks(x_m)
    ax5.set_xticklabels(metrics, color=TXT, fontsize=9)
    ax5.set_ylim(0, 1.15)
    ax5.set_ylabel("Score")
    ax5.legend(facecolor=PAN, labelcolor=TXT, fontsize=8)
    ax_style(ax5, "Full Metrics: Clean vs Drift vs Healed")

    fig.suptitle(
        "Self-Healing Neural Networks v4  ·  Constraint-Induced Plasticity\n"
        "Class-Weighted Lagrangian Healing  +  FIDI Z-Score  +  "
        "Semi-Supervised Reflexive Loop  (PyTorch)",
        color=TXT, fontsize=11, fontweight="bold", y=1.01
    )

    out = "self_healing_results_v4.png"
...
def main():
    print("=" * 62)
    print("  SELF-HEALING NEURAL NETWORKS  v4.0")
    print("  Class-Weighted Healing + FIDI + Reflexive Loop")
    print("=" * 62)
    ...
    print(f"    Fraud  : {y_train.mean().item()*100:.1f}%")
    print(f"\n    V14 clean | mean={v14_c.mean():.3f}  "
          f"std={v14_c.std():.3f}  "
    ...
    print(f"    pos_weight     : {CFG['pos_weight']}× fraud upweight")
  ...
  ──────────────────────────────────────────────────────────
  ① V14 distribution stats        →  "Simulating Real Drift"
  ...
  ⑦ Full metrics bar chart        →  "Results" (hero image)
  ⑧ Recovery delta + F1 gain      →  Abstract + Conclusion
  ──────────────────────────────────────────────────────────
    """)
